...
Vector2 GetRepulsiveForse(GraphConfig* gc, Vector2 point, qtID node, int size){
    Vector2 force = {0,0};
    ...
    if(((size/dist)<=gc->superNodeRatio)||(GETQTNODES(graph)[node].isLeaf)){
        ...
        if((GETQTNODES(graph)[node].isLeaf)&&(GETQTNODES(graph)[node].children[0] != -1)){
            Vector2 childForce = GetRepulsiveForse(gc, point,GETQTNODES(graph)[node].children[0],size/2);
        ...
        if(GETQTNODES(graph)[node].children[i] != -1){
            Vector2 childForce = GetRepulsiveForse(gc, point,GETQTNODES(graph)[node].children[i],size/2);
            force.x += childForce.x;
            force.y += childForce.y;
        }
    }
...
void ApplyForces(GraphConfig* gc, nodeID node){
    ...
    Vector2 force = GetRepulsiveForse(gc, GETNODES(graph)[node].pos, 0,gc->bounds.width);
    /*Vector2 force = {0,0};
...
void CountNodesAndEdges(GraphConfig *gc){
    gc->verticesCount = 0;
    gc->edgesCount = 0;
    for(int i = 0; i < graph->nodes.filled; ++i) {
        if (GETNODES(graph)[i].state) {
